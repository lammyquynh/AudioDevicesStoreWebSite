...
<?php
	...
	{
		if(!empty($_POST['tentaikhoan']) && !empty($_POST['matkhau']) && !empty($_POST['nhaplaimatkhau']) && 
		!empty($_POST['diachi']) && !empty($_POST['sdt']) && !empty($_POST['email']))
		{
			if($_POST['matkhau'] == $_POST['nhaplaimatkhau'])
			{
				...
				{
					if($row['taikhoan'] == $_POST['tentaikhoan'])
						$c = 1;//da ton tai
				}
				if($c == 0)
				{
					$sql='INSERT INTO admin(taikhoan,matkhau,ten,DiaChi,DienThoai,Email,quyen) VALUES ("'.$_POST['tentaikhoan'].'","'.$_POST['matkhau'].'","'.$_POST['tennguoisd'].'","'.$_POST['diachi'].'","'.$_POST['sdt'].'","'.$_POST['email'].'","'.$_POST['quyen'].'")';
					mysql_query($sql);
					$thongbao="Đăng ký thành công";
				}
				else
				{
					$thongbao = "Tên tài khoản này đã tồn tại ! xin nhập lai";
				}
			}
			else
			{
				$thongbao = "Mật khẩu nhập lại không trùng nhau !";
			}
		}
		else
		{
   	...
   	<div class="khung1">
    	...
    	<table class="x" cellpadding="0" cellspacing="0">
        	...
        	<tr>
                ...
                <td align="right"><input name="tentaikhoan" type="text" value="<?php if(isset($_POST['tentaikhoan'])) 
				echo $_POST['tentaikhoan'];?>" /></td>
            </tr>
            <tr>
            	<td align="right" bgcolor="#FFFFCC">Mật khẩu:</td>
                <td align="right"><input name="matkhau" type="text" value="<?php if(isset($_POST['matkhau'])) 
				echo $_POST['matkhau'];?> "/></td>
            </tr>
            <tr>
            	<td align="right" bgcolor="#FFFFCC">Nhập lại mật khẩu:</td>
                <td align="right"><input name="nhaplaimatkhau" type="text" value="<?php if(isset($_POST['nhaplaimatkhau'])) 
				echo $_POST['nhaplaimatkhau'];?>" /></td>
            </tr>
            <tr>
            	<td align="right" bgcolor="#FFFFCC">Tên người sử dụng</td>
                <td align="right"><input name="tennguoisd" type="text" value="<?php if(isset($_POST['tennguoisd'])) 
				echo $_POST['tennguoisd'];?>" /></td>
            </tr>
            <tr>
            	<td align="right" bgcolor="#FFFFCC">Địa chỉ:</td>
                <td align="right"><input name="diachi" type="text" value="<?php if(isset($_POST['diachi'])) 
				echo $_POST['diachi'];?> "/></td>
            </tr>
            <tr>
            	<td align="right" bgcolor="#FFFFCC">Số điện thoại</td>
                <td align="right"><input name="sdt" type="text" value="<?php if(isset($_POST['sdt'])) 
				echo $_POST['sdt'];?>" /> </td>
            </tr>
            <tr>
            	<td align="right" bgcolor="#FFFFCC">Email:</td>
                <td align="right"><input name="email" type="text" value="<?php if(isset($_POST['email'])) 
				echo $_POST['email'];?>" /></td>
            </tr>
             <tr>
            	<td align="right" bgcolor="#FFFFCC">Quyền:</td>
                <td align="left"><select name="quyen">
                					<option value="1" <?php if(isset($_POST['quyen']))
									{
														if($_POST['quyen']==1)
														{	
                					...
                					<option value="0" <?php if(isset($_POST['quyen']))
									{
														if($_POST['quyen']==0)
														{	
															echo 'selected=\"selected\"';
        ...
        <div class="them"><input type="submit" value="Thêm tài khoản" name="btnthem" /></div>
         </form>
    </div>
   </td>
  </tr>
